...
local FighterHud = Class(Screen, function(self)
...
function FighterHud:CreateOverlays(owner)
...
--11-25-22 ADDING IN FOR COMPATIBILITY
function FighterHud:OpenSpellWheel(invobject, items, radius, focus_radius)
	--GUTTED
end

function FighterHud:CloseSpellWheel(is_execute)
	--GUTTED
end

function FighterHud:IsSpellWheelOpen()
	return false --GUTTED
end

function FighterHud:GetCurrentOpenSpellBook()
	return nil --GUTTED --self.controls.spellwheel.invobject
end

function FighterHud:OnDestroy()
...
function FighterHud:OnLoseFocus()
...
function FighterHud:OnGainFocus()
...
function FighterHud:Toggle(targetindicators)
    if self.shown then
        self:Hide()
...
function FighterHud:Show()
    self.shown = true
    self.root:Show()
end

function FighterHud:GetFirstOpenContainerWidget()
...
function FighterHud:CloseContainer(container, side)
...
function FighterHud:OpenContainer(container, side)
    if container == nil then
        return
    elseif side and (TheInput:ControllerAttached() or Profile:GetIntegratedBackpack()) then
        self.controls.inv.rebuild_pending = true
...
function FighterHud:TogglePlayerAvatarPopup(player_name, data, show_net_profile, force)
...
function FighterHud:ShowEndOfMatchPopup(data)
...
function FighterHud:OpenScreenUnderPause(screen)
...
function FighterHud:OpenItemManagerScreen()
    --Hack for holding offset when transitioning from giftitempopup to wardrobepopup
    TheCamera:PopScreenHOffset(self)
...
function FighterHud:CloseItemManagerScreen()
    --Hack for holding offset when transitioning from giftitempopup to wardrobepopup
    TheCamera:PopScreenHOffset(self)
...
function FighterHud:OpenWardrobeScreen(target)
    --Hack for holding offset when transitioning from giftitempopup to wardrobepopup
    TheCamera:PopScreenHOffset(self)
...
function FighterHud:CloseWardrobeScreen()
...
function FighterHud:OpenGroomerScreen(target,filter)
...
function FighterHud:CloseGroomerScreen()
    local activescreen = TheFrontEnd:GetActiveScreen()
...
function FighterHud:OpenCookbookScreen()
...
function FighterHud:CloseCookbookScreen()
...
function FighterHud:OpenPlantRegistryScreen()
...
function FighterHud:ClosePlantRegistryScreen()
...
function FighterHud:SetRecentGifts(item_types, item_ids)
    if self.recentgiftstask ~= nil then
...
--Helper for transferring data between screens when transitioning from giftitempopup to wardrobepopup
function FighterHud:ClearRecentGifts()
    if self.recentgiftstask ~= nil then
        self.recentgiftstask:Cancel()
        self.recentgiftstask = nil
    end
...
function FighterHud:RefreshControllers()
...
function FighterHud:ShowWriteableWidget(writeable, config)
...
function FighterHud:CloseWriteableWidget()
...
function FighterHud:GoSane()
    self.vig:GetAnimState():PlayAnimation("basic", true)
end

function FighterHud:GoInsane()
    self.vig:GetAnimState():PlayAnimation("insane", true)
end

function FighterHud:GoEnlightened()
    self.vig:GetAnimState():PlayAnimation("basic", true)
end
...
function FighterHud:SetMainCharacter(maincharacter)
    if maincharacter then
		...
		--1-3-22 LETS TRY OUR OWN LITTLE SET OF Controls
		self.smashpanel = self.root:AddChild(Widget("sidepanel"))
		
		self.smashpanel:SetPosition(-80, -60, 0)
		
		
		-- self.topright_root = self:AddChild(Widget("side"))
		self.smashpanel:SetScaleMode(SCALEMODE_PROPORTIONAL)
		-- self.smashpanel:SetHAnchor(ANCHOR_RIGHT)
		self.smashpanel:SetHAnchor(ANCHOR_MIDDLE)
		self.smashpanel:SetVAnchor(ANCHOR_TOP)
		-- self.smashpanel:SetMaxPropUpscale(MAX_HUD_SCALE)
		local hudscale = 0.6
		self.smashpanel:SetScale(hudscale,hudscale,hudscale)
		
		
		--1-19-22 FIGHTERSELECT IS A SCREEN NOW! GOTTA ADD IT IN LIKE A BIG BOY.
		--THIS IS NOW HOW TO ADD IT IN LIKE A BIG BOY BUT IT'S CLOSE ENOUGH
		--[[
		self.owner:DoTaskInTime(0.5, function() 
			...
			self.smashpanel.fighterselect = self.smashpanel:AddChild(FighterSelect(self.owner))
			self.smashpanel.fighterselect:Hide()
			self.smashpanel.fighterselect:MoveToBack()
		end)
		]]
		
		--[[
		self.status = self.smashpanel:AddChild(StatusDisplays(self.owner))
		-- self.status:SetPosition(0,-110,0)
		self.status:SetPosition(0,0,0)
		-- self.status:SetScale(10,10,10)
		]]
...
function FighterHud:OnUpdate(dt)
...
function FighterHud:CloseControllerInventory()
    if self:IsControllerInventoryOpen() then
...
function FighterHud:HasInputFocus()
...
function FighterHud:SetModFocus(modname, focusid, hasfocus)
    if hasfocus then
...
function FighterHud:CloseControllerCrafting()
    if self:IsControllerCraftingOpen() then
        TheFrontEnd:GetSound():PlaySound("dontstarve/HUD/craft_close")
    end
...
function FighterHud:InspectSelf()
...
function FighterHud:OnControl(control, down)
    if FighterHud._base.OnControl(self, control, down) then
        return true
...
function FighterHud:OnRawKey(key, down)
    if FighterHud._base.OnRawKey(self, key, down) then
        return true
...
function FighterHud:UpdateDrops(camera)
...
function FighterHud:UpdateClouds(camera)
...
function FighterHud:AddTargetIndicator(target, data)
...
function FighterHud:HasTargetIndicator(target)
...
function FighterHud:RemoveTargetIndicator(target)
    if not self.targetindicators then return end
...
function FighterHud:ShowPopupNumber(val, size, pos, height, colour, burst)
    self.popupstats_root:AddChild(PopupNumber(self.owner, val, size, pos, height, colour, burst))
end

function FighterHud:ShowRingMeter(pos, duration, starttime)
...
function FighterHud:HideRingMeter(success, duration)
    if self.ringmeter ~= nil then
        if success then
...
function FighterHud:SetServerPaused(paused)
...
function FighterHud:OffsetServerPausedWidget(serverpausewidget)
