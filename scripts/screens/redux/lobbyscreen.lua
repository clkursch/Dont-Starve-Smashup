...
local CharacterSelectPanel = Class(LobbyPanel, function(self, owner)
	...
	if TheNet:GetServerGameMode() == "lavaarena" then
	    self:SetPosition(3000, 170)
	else
	    self:SetPosition(3000, 100) --LOL JUST ADDED A HUGE NUMBER TO THE X VALUE SO THAT MUTILATED PUPPET SPAWNS WAY OFF SCREEN
	end
    ...
    function self:OnGainFocus()
		if owner.currentcharacter ~= nil then
			-- self.character_scroll_list:RefocusCharacter(owner.currentcharacter) --CRASHES THE GAME
		end
    end
	...
	--11-10-17 THIS UPDATE HASNT EVEN BEEN OUT FOR TWO DAYS AND IM ALREADY TEARING INTO THE FILES.
	--WELL, I KINDA NEED TO IF I WANT TO PLAY MY MOD. LETS SEE HOW EASILY I CAN JUST SHOVE MY OLD CODE INTO HERE AND GET AWAY WITH IT
	--DST CHANGE-- SKIP THE CHARACTER SELECT PROCESS
	self.inst:DoTaskInTime(1, function() 

		-- self:SelectPortrait() --OH, FORGOT THIS TOO --11-10-17 HALF OF THESE FNS ARENT USED IN THIS VERSION
		OnCharacterClick("wilson") --MAYBE THIS IS THE RIGHT ONE HERE
		
		--9-19-17 THIS CAN BE USED TO REPLACE THE ABOVE LINE WITH MANUALY CHOSEN CHARACTERS
		owner.currentcharacter = "wilson" 
		-- self.dressup:SetCurrentCharacter("wilson") --DONT THINK THESE ARE USED IN THIS UPDATED VERSION
		-- self.dressup:UpdatePuppet()
		
		local reincarne = Reincarne --GLOBAL VARIABLE DECLARED IN MODMAIN AND SET BY CLICKING A CHARCTER SELECT SCREEN BUTTON. ONLY CLIENT-SIDE NEEDS IT
				
				--10-14-21 OKAY, CHANGE OF PLANS. INSTEAD OF HAVING A "SPECTATOR" PREFAB THATS JUST A COPY OF WILSON, "SPECTATOR" WILL BE REPLACED WITH A DEFAULT
				if Reincarne == "spectator2" then
					reincarne = DefaultFighter --A NEW GLOBAL (THAT WILL BE SET TO NEWWILSON IN THE BASE MOD)
				end
				
				owner.currentcharacter = reincarne  --11-10-17 IM GOING TO TRY TO CHANGE THESE ALL TO "OWNER" INSTEAD OF SELF
				-- self.dressup:SetCurrentCharacter(reincarne)
				-- self.dressup:UpdatePuppet()
				
				--11-10-17 LOOKS LIKE WE HAVE OTHER THINGS WE NEED TO TAKE CARE OF BEFORE WE START
				...
				-- print("CAN I START THE GAME NOW?")
				-- StartGame(self) --OH. OK
				StartGame(owner)
				--WOOPS. IT ACCIDENTALY WORKED
				...
				--9-23-17 AND THEN SET IT BACK TO DEFAULT FOR THE NEXT TIME THEY RESPAWN
				Reincarne = "spectator2"
	end)
	
	--9-29-20 OK LETS HIDE THIS WHOLE THING ITS EMBARASSING
