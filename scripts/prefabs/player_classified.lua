...
--12-30-21 HAPHAZARDLY SHOVING THIS IN HERE TO QUICKLY FIX A MAJOR CRASH WITH CONTROLLERS
...
local function OnHungerDirty(inst)
	...
	-- local selectscreen = inst._parent.HUD.controls.sidepanel:AddChild(TestScreen(inst._parent)) --owner
	
	-- inst._parent:PushEvent("show_select_screen") --5-6 THIS IS THE REAL ONE THAT HAD IT WORKING QUITE NICELY
end
...
local function OnPlayerCameraShake(inst)
    if inst._parent ~= nil and inst._parent.HUD ~= nil then
		...
		--DST TEST--
		-- ShakeAllCameras(0, duration, speed, scale)
		ShakeAllCameras(0, (inst.camerashaketime:value() + 1) / 16, (inst.camerashakespeed:value() + 1) / 256, (inst.camerashakescale:value() + 1) / 32)
    end
end
...
local function OnPlayerHUDDirty(inst)
    if inst._parent ~= nil and inst._parent.HUD ~= nil then
		...
		--1-3-22 I THINK CLASSIFIED NEEDS THESE TOO
		inst._parent.HUD.controls.status.heart2:Hide() --ALMOST...
		inst._parent.HUD.controls.status.lives:Hide() --THEEEERE WE GO. ALL CLEANED UP!!
...
--DST CHANGE-- TAKING STUPID ALL THE WAY
local function OnShowSelectScreen(inst)
    if inst._parent ~= nil then
        -- inst._parent:PushEvent("sandstormlevel", { level = inst.sandstormlevel:value() / 7 })
		-- local TestScreenr = require "screens/testscreenr"
		-- TheFrontEnd:PushScreen(TestScreenr())
		
		-- inst._parent.HUD:CloseWardrobeScreen() --OOH LOOK AT THIS NET (THAT I JUST FOUND)
		
		--WIDGET VERSION
		local TestScreen = require "widgets/testscreen"
		local selectscreen = inst._parent.HUD.controls.sidepanel:AddChild(TestScreen(inst)) --owner
		
		inst._parent:PushEvent("show_select_screen")
    end
end


--DST CHANGE-- LETS TRY A VERSION THAT MIMICS THE GIFT POPUP
local function OnShowSelectScreenPopUpDirty(inst) --NEEDS BOTH AN "ON" AND "DIRTY"
        -- elseif not inst._parent.HUD:OpenItemManagerScreen() then --THIS WAS THE ACTUAL LINE THAT DID IT. DISGUISED AS AN IF STATEMENT
	-- inst._parent:PushEvent("show_select_screen")
	-- local TestScreen = require "widgets/testscreen"
	-- local selectscreen = inst._parent.HUD.controls.sidepanel:AddChild(TestScreen(inst)) --
	--IT ALWAYS SAYS HUD NOT DEFINED????  --OOOHHHHHH... I THINK I JUST FIGURED IT OUT 
	...
	if inst._parent ~= nil and inst._parent.HUD ~= nil then
        ...
        -- elseif not inst._parent.HUD:OpenWardrobeScreen(inst.wardrobetarget:value()) then
            -- if not TheWorld.ismastersim then
                -- SendRPCToServer(RPC.CloseWardrobe)
            -- else
                -- inst._parent:PushEvent("ms_closewardrobe")  --OOHHHH MS STANDS FOR MASTER SIM
            -- end
        -- end
		...
		-- inst._parent:PushEvent("show_select_screen") --THIS ONE WORKS!! ADDING A NEW SCREEN EVERY TIME WILL NOT!!
		
		--BUT IT ONLY OPENS ONCE. LETS SEE IF THIS FIXES IT
		if not inst.isselectvisible:value() then
			-- print("I HAVE NO VALUE")
		else
			inst._parent:PushEvent("show_select_screen")
			-- print("I HAVE A VALUE!! WHERE AM I")
		end
		
		-- inst.isselectvisible:set(false) --OH PLEASE TELL ME THIS IS IT --WELL, IT AT LEAST SEEMS TO TRIGGER A HOST RESPONSE
		
		-- inst:ListenForEvent("isselectvisibledirty", OnShowSelectScreenPopUpDirty)
		
		-- inst._parent.HUD:OpenWardrobeScreen(inst.wardrobetarget:value())
    end
end

--TAKING THIS BACK FROM GIFTRECEIVER
--[[

--DST TEST ONLY!!!--
function GiftReceiver:SelectOpen(canattack)
    if self.classified ~= nil then
        -- self.classified.canattack:set(canattack)
		self.classified.isselectvisible:set(false) --???
    end
end
...
local function ShowSelectScreenPopUp(inst, show)
    -- inst.isselectvisible:set(show) --THIS PART IS FOR THE CLIENT --GAAAAAASP-I THINK I GOT IT
	OnShowSelectScreenPopUpDirty(inst)
	
	-- print("SHOW SELECT SCREEN POPUP")
	
	
	--9-5-17 --NEW ADVANCED DETECTION TO AVOID SHOWING UP WHEN NOT NEEDED
	-- if show == "force_off" then
		-- return end
	
	inst.isselectvisible:set(false) --OHHH I SEE YOU. IT WORKS BECAUSE IT CHANGED, BUT SHOWS NO DISPLAY BECAUSE ITS FALSE
	inst.isselectvisible:set(true)	--OH MY GOD I THINK THIS DID IT. IT WORKS --BUILD 0.239  5-14-17
	
	-- inst.isgiftitempopupvisible:set(show)
	-- inst.iswardrobepopupvisible:set(show)
    -- OnIsGiftItemPopUpVisibleDirty(inst)
	
	-- inst.iswardrobepopupvisible:set(show) --THIS APPARENTLY MAKES THE SCREEN SHOW UP ONCE FOR CLIENT ONLY???
    -- -- inst.wardrobetarget:set(target) --THIS BREAKS IT, BUT ISNT NEEDED
    -- OnIsWardrobePopUpVisibleDirty(inst)
end
...
local function RegisterNetListeners(inst)
    if TheWorld.ismastersim then
		...
		-- 5-14-17 (5-13) DST- HERE WE GO, MORE GIFT GIVER VERSION SELECT SCREEN
		inst:ListenForEvent("isselectvisibledirty", OnShowSelectScreenPopUpDirty) --I REALLY HAVE NO IDEA HOW THIS WORKS
	...
	-- 5-6-17 //SIGH// ALRIGHT, IF I'M GONNA BE STUPID, I HAVE TO TAKE STUPID ALL THE WAY --DST CHANGE--
	-- inst:ListenForEvent("show_select_screen", OnShowSelectScreen) --1-20-22 ATTEMPTING TO MOVE TO COMMON
...
local function fn()
	...
	--DST CHANGE-- GIFT RECIEVER VERSION --...I...DONT KNOW WHAT THIS IS
	inst.isselectvisible = net_bool(inst.GUID, "giftreceiver.isselectvisible", "isselectvisibledirty")
	...
	inst.busyremoteoverridelocomote = net_bool(inst.GUID, "locomotor.busyremoteoverridelocomote") --8-6-23 CRASH FIX. DANG I REALLY LIVE LIKE THIS HUH? WHAT A MESS
	...
	--12-30-21 SOME NEW CHANGES THAT I GUESS ARE NEEDED TO PREVENT CRASHES.
	--I BET THERES A BETTER WAY TO DO THIS (AS IN I DON'T NEED TO MAINTIAN THIS STUPID REPLICA OF PLAYERCLASSIFIED) BUT NEED A QUICK FIX FOR NOW
	...
	inst.ShowSelectScreenPopUp = ShowSelectScreenPopUp --DST CHANGE!! GIFT POPUP VERSION
