        ...
        if c_sel() ~= nil then
            ...
            elseif c_sel():HasTag("player") then
                c_sel():ListenForEvent("onattackother", function(inst)
                    print("I DID ATTTACCCCKED") --WHO DID THIS??!! WHO ON THE KLEI TEAM MADE THIS?! I SWEAR I THOUGHT IT WAS MINE, ITS EXACTLY LIKE SOMETHING I WOULD TYPE
                end)
            end
        end
    end
...
AddGlobalDebugKey(KEY_R, function()
    if TheInput:IsKeyDown(KEY_CTRL) then
        if TheInput:IsKeyDown(KEY_SHIFT) then
            -- c_regenerateworld() --PLEASE NEVER DO THIS
			-- c_reset()
			StartNextInstance() --THIS IS CLOSER TO DEFAULT THO
        else
			c_reset()
			-- StartNextInstance() --THIS ONES BETTER
        end
    else
        -- c_repeatlastcommand() --NO, I NEED THE R KEY
    end
    return true
end)
...
--12-19 IM ADDING MY OWN STEP BUTTON
AddGlobalDebugKey(KEY_P, function()
	
	print("----STEP----")
	TheSim:SetTimeScale(1)
	
	local player = ThePlayer --NEEDS TO BE AN ENTITY. CANT USE DOTASKINTIME ON THESIM
	player:DoTaskInTime(0, function()
		print("----STEP2----")
		-- TheSim:SetTimeScale(0.05)
		TheSim:SetTimeScale(0)
	end)

    -- print("Home key pressed STEPPING")
	-- TheSim:Step() --MMMM NO THANKS
	--return true
end)
...
AddGameDebugKey(KEY_I, function() --OH LOOK, I CAN OPEN GIFTS ON STAGE NOW
    ...
    elseif TheInput:IsKeyDown(KEY_SHIFT) then
		...
		ThePlayer:PushEvent("ms_opengift")
        return true
    end
end)
...
AddGameDebugKey(KEY_H, function()
	...
	end --11-27-21 ADDING A CUSTOM END HERE
    -- elseif TheInput:IsKeyDown(KEY_ALT) then
...
--LOL, I WOULD LOVE TO SEE THIS HAPPEN, BUT I NEED THIS KEY
